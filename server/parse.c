...
/*
 * 解析浏览器的请求头部，这里是为了区分用户点击搜索还是点击了搜索结果。
 * 前端要求：当用户点击搜索时，需发送<< POST >>方法；点击搜索结果时，需发送<< GET >>方法。
 * 还有一点要求，将用户的行为放置在HTTP请求的头部，不要放在Body中！！
 * 例如：
 * 用户点击一道菜名：红烧肉，浏览器需向服务器发送请求 ：
 * GET /http://home.meishichina.com/recipe-247399.html HTTP/1.1\r\n\r\n
 * 用户点击搜索框时，假如用户输入: 如何做红烧肉?
 * POST / HTTP/1.1\r\n\r\n
 * search_text = "如何做红烧肉?"
 * 这样做简化了服务器的解析，减轻了压力。而且用户发送的数据不多，放在头部就行了
 */
