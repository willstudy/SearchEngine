...
////////////////////////////////////////
// http version
void HttpVersion_print(int fd, enum HttpVersion_t v)
{
  char *s;
...
////////////////////////////////////////
// header
Header_t Header_new (char *key
		     , char *value
		     , Header_t next)
{
  Header_t p = malloc (sizeof(*p));
  p->key = key;
  p->value = value;
  p->next = next;
  return p;
...
char *Header_search (Header_t header
		     , char *key)
{
  while (header){
...
void Header_print (int fd, Header_t h)
{
  while (h){
    write (fd, h->key, strlen(h->key)+1);
    write (fd, " ", 1);
    write (fd, h->value, strlen(h->value)+1);
    write (fd, "\n", 1);
    h = h->next;
  }
  return;
...
void Http_print (int fd, Http_t http)
{
  ...
  if (http->respLine)
    RespLine_print (fd, http->respLine);
  if (http->headers){
    Header_print (fd, http->headers);
  }
  if (http->body){
    Body_print (fd, http->body);
  }
