...
"""
用于计算split_unique_title.txt中每个词条之间的关联度，
其结果保存在associate_title.txt中，格式如下：
 
豆腐:肉末:62:白菜:58:鸡蛋:33:鲫鱼:46:鱼头:41:家常:65
豆腐分别与肉末、白菜、鸡蛋、鲫鱼、鱼头、家常相关联
后面的数字：表示豆腐与其中之一词条同时出现在多少道菜中
"""
...
with open( '../data/material.txt', 'r' ) as dish_file:
	for line in dish_file.readlines():
		line.strip()
		dish_title.append( line )
dish_file.close()

with open( '../data/material_idf.txt', 'r' ) as word_file:
...
with open( '../data/material_associate.txt','w+' ) as associate_file:
