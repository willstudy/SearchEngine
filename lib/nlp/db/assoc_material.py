...
"""
用于计算split_unique_material.txt中每个词条之间的关联度，
其结果保存在associate_material.txt中，格式在assoc_material.py中有说明
"""
...
# 处理之后的material_strip.txt 把食材之间的空格去掉
with open( 'material_strip.txt', 'r' ) as dish_file:
	for line in dish_file.readlines():
		line.strip()
		dish_title.append( line )
dish_file.close()

with open( 'split_unique_material.txt', 'r' ) as word_file:
...
with open( 'associate_material.txt','w+' ) as associate_file:
